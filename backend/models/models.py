...
class User(db.Model):
    ...
    free_generation_used = db.Column(db.Integer , deafult=0)
...
class PantryItem(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True) , primary_key=True , default=uuid.uuid4)
    user_id = db.Column(UUID(as_uuid=True) , db.ForeingKey("user.id",ondelete="CASCADE") , nullable=False , index=True)
    ...
    created_at = db.Column(db.DateTime , default=datetime.utcnow)
    user = db.relationship("User" , back_populates="pantry_item")
